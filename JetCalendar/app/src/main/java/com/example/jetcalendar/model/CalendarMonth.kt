...
data class CalendarMonth(
    ...
    val days = mutableListOf<CalendarDay>().apply {
        ...
        for (i in 1..startDayOfWeek.ordinal) {
            add(CalendarDay("", DaySelectedStatus.NonClickable))
        }

        // add days of the month
        for (i in 1..numDays) {
            add(CalendarDay(i.toString(), DaySelectedStatus.NoSelected))
        }
    }.toList()

    fun getDay(day: Int): CalendarDay = days[day + startDayOfWeek.ordinal - 1]


    fun getPreviousDay(day: Int): CalendarDay? {
        if (day <= 1) return null
        return getDay(day = day - 1)
    }
    ...
    private fun completeWeek(list: List<CalendarDay>): List<CalendarDay> {
        ...
        return if (gapsToFill != 0) {
            ...
            while (gapsToFill > 0) {
                mutableList.add(CalendarDay("", DaySelectedStatus.NonClickable))
                gapsToFill--
            }
            mutableList
        } else {
